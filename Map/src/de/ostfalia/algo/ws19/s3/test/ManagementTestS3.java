...
//		String msg = "Erhaltene Rueckgabe beim Einfuegen des Datensatzes.";
		for (int i = 0; i < data10.length; i++) {
			...
			mgnt.insert(member);
//			subTrace.addInfo(PassTrace.ifTrue(msg, mgnt.insert(member)));
		}
...
//			subTrace.addInfo(PassTrace.ifTrue(msg, mgnt.insert(member)));
	...
	public void testHeight() throws IOException {
		...
		int exp = 4;		
		trace.add(PassTrace.ifEquals("Hoehe des binaeren Suchbaums muss %d betragen.",
			      exp, mgnt.height(), exp));
